...
Partial Class admin_forgot_password
    ...
    Protected Sub btn_Reset_Click(sender As Object, e As System.EventArgs) Handles btn_Reset.Click
        ...
        Dim strSQL As String = "SELECT User_ID, User_First_Name, User_Last_Name, User_Email FROM User_Master_File WHERE (User_Email = '" & login_email.Text & "')"
        objConn.Open()
        ...
        If objRead.Read() Then
            Session("u_i_d") = objRead.Item("User_ID")
        ...
        Dim strSQL1 As String = "UPDATE User_Master_File SET User_Password = @Password WHERE (User_ID = '" & Session("u_i_d") & "')"
        objConn1.Open()
        Dim objComm1 As New Data.SqlClient.SqlCommand(strSQL1, objConn1)
        objComm1.Parameters.Add(paramPwd)
        objComm1.ExecuteNonQuery()

        UserLogin.AdminLog(Session("login_email"), "UPDATE", "User_Master_File", "Reset User Password for User_ID : " & Session("u_i_d"))
