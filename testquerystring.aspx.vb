...
<System.Web.Script.Services.ScriptMethod()> _
	...
	public function resendEmailfor() as integer
        ...
        Dim strSQL As String = "SELECT * FROM Donor_Master_File WHERE Donor_ID between 285 and 299 and Donor_VID is not null "
        objConn.Open()
		...
		While objRead.Read()
			emailaddress =objRead.Item("Donor_Email")
			randID =objRead.Item("Donor_VID")
			username=objRead.Item("Donor_First_Name")
			username += " " & objRead.Item("Donor_Last_Name")
			ID = ID+1
			lblerrormessage.text +=" "& vbCrLf &" " &emailaddress &" " &randID & "  "& username & " </BR>"
			
			'If emailaddress.Contains("jnbs.com") Then
		   'lblerrormessage.text+=" CONTAINS JNBS \N"
		   'ELSE
		   ''lblerrormessage.text+=" DOES NOT CONTAIN JNBS "
			'End if
			email( emailaddress, randID ,username)
        end while
		lblerrormessage.text +="COUNT NUMBER OF MAIL : " & ID
		return 1
	end function
	
	public function email(ByVal emailaddress As String,ByVal  randID As String, ByVal  username As String) as integer 
	dim FromEmail as string
	
	 Dim sMsg As String
        Dim sLnk As String = "https://www.isupportjamaica.com/activate_account.aspx?aaid=" & randID & "&aa_email=" & emailaddress
        sMsg = "Hello "& username & vbCrLf & "<br>"
        sMsg += "" & vbCrLf & "<br>"
        ...
        Try
            ...
            EmailManager.SendEmail(sMsg, "I Support Jamaica confirmation email.", emailaddress, "New User", "I Support Jamaica Team <info@isupportjamaica.com>", Nothing)

				lblerrormessage.text +="Sent to client : "& vbCrLf &" " &emailaddress &" " &randID & "  "& username & " </BR>"
        Catch EXC As Exception
            Trace.Warn(EXC.Message)
            ''MsgBox(EXC.Message)
			lblerrormessage.text=EXC.Message
        Finally
            lblerrormessage.Text += "final"
        End Try
		
		 sMsg = Nothing
            sMsg = "Hi Admin " & vbCrLf & "<br>"
            sMsg += "" & vbCrLf & "<br>"
            sMsg += "This is to inform you that" & username & " with email address " & emailaddress & " have registered an account with isupportjmaica.com" & vbCrLf & "<br>"
            sMsg += "" & vbCrLf & "</br>"
            sMsg += "Thank you  </br>"
            ...
            EmailManager.SendEmail(sMsg, "I Support Jamaica confirmation email.", "ISupportJamaica@jnbs.com", "Admin Confirmation", "I Support Jamaica Team <info@isupportjamaica.com>", Nothing)
				...
				lblerrormessage.text +="Sent to Admin : "& vbCrLf &" " &emailaddress &" " &randID & "  "& username & " </BR>"
            Catch ex As Exception
	...
	''resendEmailfor()
        ...
        'Try
        '    NetMail.From = New Net.Mail.MailAddress("o.waisome@epohjm.com")
        '    NetMail.To.Add(New Net.Mail.MailAddress(yourEmail.Text, "New User"))

        '    NetMail.IsBodyHtml = True
        '    NetMail.Subject = "I Support Jamaica confirmation email."
        '    NetMail.Body = sMsg
